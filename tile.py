...
class Tile(Compound):
    ...
    def create(cls, label=None):
        ...
        m.top_male_port.applyTransformation(CoordinateTransform.translation((0,0.5,0)))

        m.add(Port.create(),'left_male_port')
        m.left_male_port.applyTransformation(CoordinateTransform.rotation_around_z(pi/2))
        m.left_male_port.applyTransformation(CoordinateTransform.translation((-0.5,0,0)))

        m.add(Port.create(),'bottom_female_port')
        m.bottom_female_port.applyTransformation(CoordinateTransform.translation((0,-0.5,0)))

        m.add(Port.create(),'right_female_port')
        m.right_female_port.applyTransformation(CoordinateTransform.rotation_around_z(pi/2))
        m.right_female_port.applyTransformation(CoordinateTransform.translation((0.5,0,0)))
