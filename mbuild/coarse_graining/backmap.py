...
The CG system — an ``mbuild.path.Path`` or an ``mbuild.Compound``
whose leaf particles are the beads — is handed to CGsmiles as the
lowest-resolution graph and resolved to molecular detail:

1. Convert the CG bond graph to a CGsmiles-compatible meta graph where
   bead names become fragment names (``to_cgsmiles_graph``).
2. Resolve the meta graph to a molecular (atomistic) graph using
   CGsmiles fragment strings with SMILES + bonding descriptors and/or
   tagged mBuild compounds (``resolve``).
3. Convert the resolved molecular graph back to an atomistic
...
If you use this feature please cite the paper referenced in
``mbuild.coarse_graining``.
"""
...
from .convert import to_cgsmiles_graph
...
def resolve(
    ...
    templates : dict[str, mbuild.Compound], optional
        ...
        every fragment used by the system.
...
def backmap(
    ...
    Resolves the system's bond graph to an atomistic molecule via
    ...
    compound = _molecule_to_compound(cg_graph, molecule, node_to_beads, positions)
...
def _molecule_to_compound(cg_graph, molecule, node_to_beads, positions):
    ...
    components = sorted(
        (sorted(component) for component in nx.connected_components(cg_graph)),
