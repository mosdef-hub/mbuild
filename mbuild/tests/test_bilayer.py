...
class TestBilayer(BaseTest):
    ...
    def test_composition(self, ternary_lipid_mix):
        bilayer = mb.Bilayer(ternary_lipid_mix, n_lipids_x=2, n_lipids_y=2, solvent_per_lipid=0)
        assert bilayer.number_of_each_lipid_per_leaflet == [2, 1, 1]
        bilayer = mb.Bilayer(ternary_lipid_mix, n_lipids_x=4, n_lipids_y=2, solvent_per_lipid=0)
    ...
    def test_bad_dimensions(self, binary_lipid_mix, ternary_lipid_mix):
        with pytest.raises(ValueError):
            mb.Bilayer(binary_lipid_mix, n_lipids_x=0, n_lipids_y=1)
        with pytest.raises(TypeError):
            mb.Bilayer(binary_lipid_mix, n_lipids_x=4.5, n_lipids_y=5)
        with pytest.raises(ValueError):
            mb.Bilayer(ternary_lipid_mix, n_lipids_x=2, n_lipids_y=1)

    def test_bad_lipids(self, dspcua, ffaua, alcua):
        with pytest.raises(TypeError):
            mb.Bilayer((dspcua, ffaua, alcua, dspcua))
        with pytest.raises(ValueError):
            mb.Bilayer((dspcua, 1.0, 0), n_lipids_x=2, n_lipids_y=2)
        with pytest.raises(ValueError):
            lipids = [(ffaua, 0.5, 0.0, 0), (alcua, 0.5, 17)]
            mb.Bilayer(lipids, n_lipids_x=2, n_lipids_y=2)
        with pytest.raises(ValueError):
            frac = [(ffaua, 0.7, 0.0, 0), (alcua, 0.5, 17)]
            mb.Bilayer(frac, n_lipids_x=2, n_lipids_y=2)
        with pytest.raises(ValueError):
            mb.Bilayer(lipids, n_lipids_x=1, n_lipids_y=1)
    ...
    def test_bad_tilt_angle(self, binary_lipid_mix):
        with pytest.raises(TypeError):
            mb.Bilayer(binary_lipid_mix, tilt_angle="35")

    def test_bad_random(self, binary_lipid_mix):
        with pytest.raises(TypeError):
            mb.Bilayer(binary_lipid_mix, max_tail_randomization="24")
        with pytest.raises(ValueError):
            mb.Bilayer(binary_lipid_mix, max_tail_randomization=150)

    def test_bad_solvent(self, binary_lipid_mix):
        with pytest.raises(TypeError):
            mb.Bilayer(binary_lipid_mix, solvent=5)
        with pytest.raises(ValueError):
            mb.Bilayer(binary_lipid_mix, solvent_per_lipid=10.5)

    def test_bad_itp(self, binary_lipid_mix):
        with pytest.raises(TypeError):
            mb.Bilayer(binary_lipid_mix, make_files=False, itp_path=24)
        with pytest.raises(IOError):
            mb.Bilayer(binary_lipid_mix, make_files=True, itp_path="bruh")

    def test_bad_files(self, binary_lipid_mix):
        with pytest.raises(TypeError):
            mb.Bilayer(binary_lipid_mix, make_files="banana", itp_path='bruh')
        with pytest.raises(TypeError):
            mb.Bilayer(binary_lipid_mix, make_files=False, filename=67)
