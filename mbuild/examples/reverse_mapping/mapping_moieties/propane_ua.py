...
class Propane_ua(mb.Compound):
    """A propane molecule with one hydrogen omitted for bonding """
    def __init__(self):
        super(Propane_ua, self).__init__()

        mb.load('propane_ua.pdb', compound=self, relative_to_module=self.__module__)
        self.translate(-self[0].pos)  # Move carbon to origin.

        self.add(mb.Port(anchor=self[-1]), 'down')
