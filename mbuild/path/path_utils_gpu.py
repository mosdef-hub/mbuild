...
This module provides drop-in replacements for some of the @njit functions in path_utils.py,
using numba's cuda.jit.
...
def check_path_split(d_static_points, dynamic_points, candidates, radius, tolerance):
    ...
    Dynamic points are those being generated by the current random walk.

    Dynamic points must be loaded onto the GPU with each iteration, static are loaded
    once at the beginning of the walk and kept there. See _prepare_gpu_static_points().

    Parameters
    ----------
    d_static_points : cuda device array
        Static points already on GPU (previous paths, included compounds). Do NOT transfer.
    dynamic_points : numpy array
        Dynamic points on CPU (points on the current path). Will be transferred.
    candidates : numpy array
        Candidate points to check.
    radius : float
        Site radius.
    tolerance : float
