    ...
    for i, atom in enumerate(structure.atoms):
        ...
        while interate_thru_names is True:
            ...
            if str(atom.name)[:1] == '_':
                if bead_to_atom_name_dict is not None and (str(atom.name) in bead_to_atom_name_dict) is True:
            ...
            atom_res_no_resname_atomname_iteration = str(residue_id_list[i]) + '_' + str(residue_names_list[i]
                                                                                         ) + '_' + atom_name_iteration

            if unique_individual_atom_names_dict.get(str(atom_res_no_resname_atomname_iteration)) is None:
                unique_individual_atom_names_dict.update({atom_res_no_resname_atomname_iteration: i + 1})
...
class Charmm:
    def __init__(self, structure_box_0, filename_box_0, structure_box_1=None, filename_box_1=None,
        ...
        if self.structure_box_1 is None and not isinstance(self.structure_box_0, Compound):
            self.input_error = True
        ...
        if self.filename_box_1 is not None and not isinstance(self.filename_box_1, str):
            self.input_error = True
        ...
        if self.fix_residue_in_box is not None and not isinstance(self.fix_residue_in_box, list):
            self.input_error = True
        ...
        if self.bead_to_atom_name_dict is not None and not isinstance(self.bead_to_atom_name_dict, dict):
            self.input_error = True
        ...
        if self.structure_box_1:
            ...
            [self.structure_box_0_ff,
             ...
             self.residues_applied_list_box_0] = specific_ff_to_residue(self.structure_box_0,
            ...
            [self.structure_box_1_ff,
             ...
             self.residues_applied_list_box_1] = specific_ff_to_residue(self.structure_box_1,
                                                                    forcefield_selection=self.forcefield_selection,
                                                                    residues=self.residues,
                                                                    reorder_res_in_pdb_psf=self.reorder_res_in_pdb_psf,
                                                                    box=self.box_1,
            ...
            self.residue_id_list = self.structure_box_0_ff + self.structure_box_1_ff
            self.combined_1_4_lj_dict_per_residue.update(self.LJ14scalar_dict_box_0)
            self.combined_1_4_lj_dict_per_residue.update(self.LJ14scalar_dict_box_1)
            self.combined_1_4_coul_dict_per_residue.update(self.coulomb14scalar_dict_box_0)
            self.combined_1_4_coul_dict_per_residue.update(self.coulomb14scalar_dict_box_1)
        ...
        if self.structure_box_1:
            ...
            self.mass_dict = dict([(self.unique_types.index(atom_type) + 1, mass
                                    ) for atom_type, mass in zip(self.types, self.masses)])

        else:
            self.masses = np.array([atom.mass for atom in self.structure_box_0_ff.atoms]
                                   ) / self.mass_conversion_factor
            self.mass_dict = dict([(self.unique_types.index(atom_type) + 1, mass
                                    ) for atom_type, mass in zip(self.types, self.masses)])
        ...
        self.box_0 = Box(lengths=np.array([0.1 * val for val in self.structure_box_0_ff.box[0:3]]
        ...
        forcefields = [atom.type + '_' + atom.residue.name for atom in self.structure_selection.atoms]
        ...
        self.sigma_dict = dict(
            [(self.unique_types.index(atom_type), sigma) for atom_type, sigma in zip(self.types, sigmas)])
        self.LJ_1_4_dict = dict(
            [(self.unique_types.index(atom_type), self.combined_1_4_lj_dict_per_residue[residues_all_list])
             for atom_type, residues_all_list in zip(self.types, residues_all_list)])
            ...
            for res_i in range(0, len(self.all_individual_atom_names_list)):
                try:
                    if self.all_individual_atom_names_list[res_i] \
                            not in self.all_res_unique_atom_name_dict[self.all_residue_names_List[res_i]]:
                        self.all_res_unique_atom_name_dict.setdefault(self.all_residue_names_List[res_i], []
                                                                     ).append(self.all_individual_atom_names_list[
                                                                                                          res_i])
                except:
                    self.all_res_unique_atom_name_dict.setdefault(self.all_residue_names_List[res_i], []
                                                                 ).append(self.all_individual_atom_names_list[res_i])

        print('all_res_unique_atom_name_dict = {}'.format(self.all_res_unique_atom_name_dict))
            ...
            with open(self.ff_filename, 'w') as data:
                ...
                if self.detect_forcefield_style:
                    ...
                    if len(self.structure_selection.urey_bradleys) > 0:
                        ...
                        data.write("! POTENTIAL ERROR: GOMC does no support the Urey bradley terms")
                ...
                if use_rb_torsions and use_dihedrals:
                    warn("Multiple dihedral styles detected, check your Forcefield XML and structure_selection")
                            ...
                            data.write(bond_format.format(base10_to_base52_alph(
                                self.atom_types_to_index_value_dict[params[2][0]+'_' + str(params[3])]),
                                                           base10_to_base52_alph(
                                                               self.atom_types_to_index_value_dict[params[2][1] 
                                                                                                   + '_' + 
                                                                                                   str(params[4])]),
                    ...
                    for params, idx in unique_angle_types.items():
                        ...
                        if (self.gomc_fix_bonds_angles is not None) and ((params[4] and params[5] and params[6])
                            ...
                            data.write(angle_format.format(base10_to_base52_alph(
                                self.atom_types_to_index_value_dict[params[3][0]+'_'+params[4]]),
                                                           base10_to_base52_alph(
                                                               self.atom_types_to_index_value_dict[params[2] + '_'
                                                                                                   + params[5]]),
                                                           base10_to_base52_alph(
                                                               self.atom_types_to_index_value_dict[params[3][1] + '_'
                                                                                                   + params[6]]),
                ...
                if dihedrals:
                    if use_rb_torsions:
                        ...
                        for params, idx in unique_dihedral_types.items():
                            ...
                            # **************************************
                            dihedral_format = '{}\t{}\t{}\t{}\t{:.6f}\t{}\t{}\t\t! {}\t{}\t{}\t{}\n'
                            data.write(dihedral_format.format(base10_to_base52_alph(
                                self.atom_types_to_index_value_dict[params[8] + '_' + params[12]]),
                                                              base10_to_base52_alph(self.atom_types_to_index_value_dict[
                                                              ...
                                                              params[10] + '_' + params[14],
                                                              params[11] + '_' + params[15]))
                            data.write(dihedral_format.format(base10_to_base52_alph(self.atom_types_to_index_value_dict[
                                                                                        params[8] + '_' + params[12]]),
                                                              base10_to_base52_alph(self.atom_types_to_index_value_dict[
                                                                                        params[9] + '_' + params[13]]),
                                                              base10_to_base52_alph(self.atom_types_to_index_value_dict[
                        ...
                        if sum(list_if_large_error_dihedral_overall) > 0:
                            list_max_error_abs_dihedral_overall = \
                ...
                if self.non_bonded_type == 'LJ':
                    ...
                    data.write("ERROR: Currently the Mie potential (non_bonded_type) is not supported in this MoSDeF "
                               "GOMC parameter writer\n")
                    print_error_message = "ERROR: Currently the Mie potential (non_bonded_type) is not " \
                                          "supported in this MoSDeF GOMC parameter writer."
                    raise ValueError(print_error_message)
                else:
                    data.write("ERROR: Currently this potential (non_bonded_type) is not supported in "
                               "this MoSDeF GOMC parameter writer\n")
                    print_error_message = "ERROR: Currently this potential (non_bonded_type) is not supported in " \
                                          "this MoSDeF GOMC parameter writer."
                    raise ValueError(print_error_message)
    ...
    def write_psf(self):
        ...
        for q in range(0, len(list_of_structures)):
            ...
            [unique_individual_atom_names_dict, 
             individual_atom_names_list, 
             missing_bead_to_atom_name] = unique_atom_naming(stuct_only_iteration, 
                                                             residue_id_list, 
                                                             residue_names_list,
                                                             bead_to_atom_name_dict=self.bead_to_atom_name_dict)
            ...
            for i_atom, PSF_atom_iteration_1 in enumerate(stuct_iteration.atoms):
                ...
                atom_type_iter = base10_to_base52_alph(
                    self.atom_types_to_index_value_dict[PSF_atom_iteration_1.type + '_' +
                                                        PSF_atom_iteration_1.residue.name])

                atom_lines_iteration = psf_formating % (i_atom + 1, segment_id, ff_name[i_atom],
            ...
            if stuct_iteration.groups is True:
                ...
                structure_abs_charge_value = abs(sum(atom_charge_iter.charge
    ...
    def write_pdb(self):
        ...
        for q in range(0, len(list_of_structures)):
            ...
            if (self.fix_residue is not None) and (self.fix_residue_in_box is not None):
            ...
            for k, atom in enumerate(stuct_only_iteration.atoms):
                residue_names_list.append(atom.residue.name)
                if (self.fix_residue is not None) and (atom.residue.name in self.fix_residue):
                    beta_iteration = 1.00
                elif (self.fix_residue_in_box is not None) and (atom.residue.name in self.fix_residue_in_box):
                    beta_iteration = 2.00
                else:
                    beta_iteration = 0.00
            ...
            if stuct_only_iteration.box is not None:
                output_write.write('CRYST1%9.3f%9.3f%9.3f%7.2f%7.'
